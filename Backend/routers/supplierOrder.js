...
router.put('/:id', async (req, res) => {
    const { id } = req.params;
    const { product_name, category, brand, model, quantity, supplier_id, supplier_name } = req.body;

    // Validate input
    if (!product_name || !category || !brand || !model || !quantity || !supplier_id|| !supplier_name) {
        return res.status(400).json({ message: 'All fields are required.' });
    }
    ...
    try {
        const updatedOrder = await SupplierOrder.findByIdAndUpdate(
            id,
            { product_name, category, brand, model, quantity: parsedQuantity, supplier_id, supplier_name },
            { new: true }
        );
