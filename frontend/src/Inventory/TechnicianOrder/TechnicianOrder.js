...
const TechnicianOrder = ({ technicianId }) => {
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      try {
        const response = await axios.get(`/api/orders/technician/${technicianId}`);
  ...
  const handleNewOrder = async () => {
    ...
    try {
      ...
      const response = await axios.post('/api/orders/check', newOrderData);

      setMessage(response.data.message);
      // Optionally re-fetch past orders after inventory update
      const updatedOrders = await axios.get(`/api/orders/technician/${technicianId}`);
